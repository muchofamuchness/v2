...
class AgentAlphaBeta(Agent):
    ...
    def alphaBeta(self, env, agent, turn, depth):
        ...
        if turn == agent:
            ...
            current_max = (math.inf if current_max[0] >= self.beta else current_max[0], current_max[1])
            ...
            current_min = (-math.inf if current_min[0] <= self.alpha else current_min[0], current_min[1])
    ...
    def process_run(self, env, agent_id, operator):
        depth = 1
        while True:
            result = self.alphaBeta(env, agent_id, agent_id, depth)
