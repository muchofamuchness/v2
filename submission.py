...
class AgentAlphaBeta(Agent):
    ...
    def alphaBeta(self, env, agent, turn, depth, time_limit, alpha, beta):
        if env.done() or depth == 0:
            return self.heuristic(env, agent)
        ...
        children_heuristics = [(self.return_value(self.alphaBeta(child, agent, 1-turn, depth - 1, alpha, beta)), op) for child, op in zip(children, operators)]

        if turn == agent:
            current_max = max(children_heuristics, key=lambda x: x[0])
            alpha = max(current_max, alpha)
            current_max = None if current_max >= beta else current_max
            return current_max
        else:
            current_min = min(children_heuristics, key=lambda x: x[0])
            beta = min(current_max, beta)
            current_min = None if current_min <= alpha else current_min
            return current_min
